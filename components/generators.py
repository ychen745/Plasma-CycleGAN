...
class PositionalEncoding3D(nn.Module):
    ...
    def forward(self, tensor, start_idx):
        """
        ...
        for b in range(batch_size):
            ...
            j = start_idx[b, 1]
            k = start_idx[b, 2]
            pos_x = torch.arange(i, i + x, device=tensor.device, dtype=self.inv_freq.dtype)
            pos_y = torch.arange(j, j + y, device=tensor.device, dtype=self.inv_freq.dtype)
            pos_z = torch.arange(k, k + z, device=tensor.device, dtype=self.inv_freq.dtype)
            ...
            emb[b, :, :, :, : self.channels] = emb_x
            emb[b, :, :, :, self.channels : 2 * self.channels] = emb_y
            emb[b, :, :, :, 2 * self.channels :] = emb_z
...
class PositionalEncodingPermute3D(nn.Module):
    ...
    def forward(self, tensor, start_idx):
        ...
        enc = self.penc(tensor, start_idx)
...
class PEGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm3d, use_dropout=False, n_blocks=6, padding_type='replicate'):
        ...
        self.ngf = ngf
        self.device = torch.device('cuda:0') if torch.cuda.is_available() else torch.device('cpu')
        self.pos_emb = PositionalEncodingPermute3D(256)
        self.encoder = PEEncoder(input_nc, output_nc, ngf=ngf, norm_layer=nn.InstanceNorm3d, use_dropout=False, n_blocks=n_blocks, padding_type='replicate')
        self.res = PERes(input_nc, output_nc, ngf=ngf, norm_layer=nn.InstanceNorm3d, use_dropout=False, n_blocks=n_blocks, padding_type='replicate')
        self.decoder = PEDecoder(input_nc, output_nc, ngf=ngf, norm_layer=nn.InstanceNorm3d, use_dropout=False, n_blocks=n_blocks, padding_type='replicate', n_condition=0)

    def forward(self, input, start_idx):
        pos = self.pos_emb(input, start_idx)
        x = input + pos
        x = self.encoder(input)
        x = self.res(x)
        x = self.decoder(x)
...
class PERes(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=16, norm_layer=nn.BatchNorm3d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        assert(n_blocks >= 0)
        super(PERes, self).__init__()
        self.input_nc = input_nc
        self.output_nc = output_nc
        self.ngf = ngf
        if type(norm_layer) == functools.partial:
...
class PEConditionGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm3d, use_dropout=False, n_blocks=6, padding_type='replicate'):
        assert(n_blocks >= 0)
        super(PEConditionGenerator, self).__init__()
        self.input_nc = input_nc
        self.output_nc = output_nc
        self.ngf = ngf
        self.pos_emb = PositionalEncodingPermute3D(256)
        ...
        self.encoder = PEConditionEncoder(input_nc, output_nc, ngf=ngf, norm_layer=nn.InstanceNorm3d, use_dropout=False, n_blocks=n_blocks, padding_type='replicate')
        self.res = PEConditionRes(input_nc, output_nc, ngf=ngf, norm_layer=nn.InstanceNorm3d, use_dropout=False, n_blocks=n_blocks, padding_type='replicate')
        self.decoder = PEConditionDecoder(input_nc, output_nc, ngf=ngf, norm_layer=nn.InstanceNorm3d, use_dropout=False, n_blocks=n_blocks, padding_type='replicate', n_condition=0)
        self.emb = DAFTBlock(256, 256, stride=1, ndim_non_img=1).to(self.device)
        # self.w = torch.randn((16, 16, 16), requires_grad=True).to(self.device)
        # self.b = torch.zeros((16, 16, 16), requires_grad=True).to(self.device)

    def forward(self, input, start_idx, condition):
        pos = self.pos_emb(input, start_idx)
        x = input + pos
        x = self.encoder(input)
        x = self.emb(x, condition)
...
class PEConditionEncoder(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=16, norm_layer=nn.BatchNorm3d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        assert(n_blocks >= 0)
        super(PEConditionEncoder, self).__init__()
        self.input_nc = input_nc
        self.output_nc = output_nc
        self.ngf = ngf
        if type(norm_layer) == functools.partial:
...
class PEConditionRes(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=16, norm_layer=nn.BatchNorm3d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        assert(n_blocks >= 0)
        super(PEConditionRes, self).__init__()
        self.input_nc = input_nc
        self.output_nc = output_nc
        self.ngf = ngf
        if type(norm_layer) == functools.partial:
...
class PEConditionDecoder(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=16, norm_layer=nn.BatchNorm3d, use_dropout=False, n_blocks=6, padding_type='reflect', n_condition=0):
        assert(n_blocks >= 0)
        super(PEConditionDecoder, self).__init__()
        self.input_nc = input_nc
        self.output_nc = output_nc
        self.ngf = ngf
        if type(norm_layer) == functools.partial:
