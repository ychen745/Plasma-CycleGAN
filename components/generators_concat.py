...
class ConditionGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm3d, use_dropout=False, n_blocks=6, padding_type='replicate'):
        ...
        self.conv1x1 = nn.Conv3d(257, 256, kernel_size=1)
        ...
        if fwd:
            if mask:
                ...
                for i in range(x.shape[0]):
                    ctbs.append(torch.full((1, 1, x.shape[2], x.shape[3], x.shape[4]), condition[i].item()))
                condition_tensor = torch.concat(ctbs, dim=0).to(self.device)
                x = torch.concat((x, condition_tensor), dim=1)
                x = self.conv1x1(x)
        x = self.res(x)
        x = self.decoder(x)
        return x
