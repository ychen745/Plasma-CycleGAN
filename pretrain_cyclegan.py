import sys
from utils.PretrainDataset import *
import utils.PretrainDataset as PretrainDataset
...
if __name__ == '__main__':
    ...
    trainTransforms = [
                PretrainDataset.RandomCrop((opt.patch_size[0], opt.patch_size[1], opt.patch_size[2]), opt.drop_ratio, min_pixel),
                ]
    if opt.cross_validation:
        train_set = PretrainDataSet(opt.data_path, True, opt.data_split, opt.fold, transforms=trainTransforms, mode='train')
        val_set = PretrainDataSet(opt.data_path, True, opt.data_split, opt.fold, transforms=None, mode='val')
    else:
        train_set = PretrainDataSet(opt.data_path, False, opt.data_split, transforms=trainTransforms, mode='train')
        val_set = PretrainDataSet(opt.data_path, False, opt.data_split, transforms=None, mode='val')
    ...
    for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):
        ...
        for i, data in enumerate(val_loader):
            ...
            real_b = pred['real_B']
            real_b = real_b.squeeze().data.cpu().numpy()
            real_b = (real_b * 127.5) + 127.5
            fake_b = pred['fake_B']
            fake_b = fake_b.squeeze().data.cpu().numpy()
            fake_b = (fake_b * 127.5) + 127.5

            ssims.append(structural_similarity_index(real_b, fake_b))
