...
from components.loss import GANLoss, Cor_CoeLoss
...
class ShareGANConditionModel(BaseModel):
    def name(self):
        return 'ShareGANConditionModel'
    ...
    def initialize(self, opt):
        ...
        self.loss_names = ['D_A', 'G_A', 'cycle_A', 'idt_A', 'D_B', 'cycle_B', 'idt_B']
        ...
        if self.isTrain:
            ...
            self.criterionGAN = GANLoss(use_lsgan=not opt.no_lsgan).cuda()
    ...
    def forward(self):
        self.fake_B = self.netG_A(self.real_A, 0.0, self.condition, True)
        self.rec_A = self.netG_A(self.fake_B, 1.0, self.condition, False)
        self.fake_A = self.netG_A(self.real_B, 1.0, self.condition, False)
        self.rec_B = self.netG_A(self.fake_A, 0.0, self.condition, True)
    ...
    def backward_G(self, update_step=1.):
        ...
        if lambda_idt > 0:
            # save resources
            self.idt_A = self.netG_A(self.real_B, 0.0, self.condition, True)
            self.idt_B = self.netG_A(self.real_A, 1.0, self.condition, True)
        ...
        # combined loss
        self.loss_G = (self.loss_G_A + self.loss_G_B + self.loss_cycle_A + self.loss_cycle_B + self.loss_idt_A + self.loss_idt_B) / update_step
