...
class ShareGANModel(BaseModel):
    ...
    def forward(self):
        self.fake_B = self.netG_A(self.real_A, 0.0)
        self.rec_A = self.netG_A(self.fake_B, 1.0)
        self.fake_A = self.netG_A(self.real_B, 1.0)
        self.rec_B = self.netG_A(self.fake_A, 0.0)
    ...
    def backward_G(self, update_step=1.):
        ...
        if lambda_idt > 0:
            # save resources
            self.idt_A = self.netG_A(self.real_B, 0.0)
            self.idt_B = self.netG_A(self.real_A, 1.0)
