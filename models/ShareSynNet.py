...
class half_PolyPhase_resUnet_Adain(nn.Module):
    def __init__(self, in_ch=1, out_ch=1):
        ...
        self.conv1x1 = nn.Conv3d(513, 512, kernel_size=1)
...
class half_PolyPhase_resUnet_Adain_condition(nn.Module):
    def __init__(self, in_ch=1, out_ch=1):
        super(half_PolyPhase_resUnet_Adain_condition, self).__init__()
        self.down1 = down_half_polyphase(in_ch=in_ch, out_ch=32)
        self.down2 = down_half_polyphase(in_ch=32, out_ch=128)
        self.down3 = down_half_polyphase(in_ch=128, out_ch=256)
        self.down4 = down_half_polyphase(in_ch=256, out_ch=512)
    ...
    def forward(self, input, alpha, condition, fwd=True):
        batch_size = input.shape[0]
        shared_code = self.adain_shared(batch_size)
        x1 = self.down1(input, shared_code, alpha)
        x2 = self.down2(x1, shared_code, alpha)
        ...
        if fwd:
            ...
            for i in range(x4.shape[0]):
                ctbs.append(torch.full((1, 1, x4.shape[2], x4.shape[3], x4.shape[4]), condition[i].item()))
            condition_tensor = torch.concat(ctbs, dim=0).to(self.device)
            x4 = torch.concat((x4, condition_tensor), dim=1)
            x4 = self.conv1x1(x4)
