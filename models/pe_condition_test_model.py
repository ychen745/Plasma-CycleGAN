...
class PEConditionTestModel(BaseModel):
    def name(self):
        return 'PEConditionTestModel'
    ...
    def initialize(self, opt):
        ...
        self.netG = networks3D.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG,
                                      opt.norm, not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids, opt.bbbm_mode)

        # assigns the model to self.netG_[suffix] so that it can be loaded
        # please see BaseModel.load_networks
        setattr(self, 'netG_A' + opt.model_suffix, self.netG)

    def set_input(self, input, start_idx, condition):
    ...
    def forward(self):
        self.fake_B = self.netG(self.real_A, self.start_idx, self.condition)
