...
from components.loss import GANLoss, Cor_CoeLoss, SSIMLoss
...
class PEModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.netG_A = networks3D.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.norm,   # nc number channels
                                        not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids, opt.bbbm_mode)
    ...
    def set_input(self, input):
        ...
        self.start_idx = input[2].to(self.device)

    def forward(self):
        self.fake_B = self.netG_A(self.real_A, self.start_idx)
        self.rec_A = self.netG_B(self.fake_B, self.start_idx)

        self.fake_A = self.netG_B(self.real_B, self.start_idx)
        self.rec_B = self.netG_A(self.fake_A, self.start_idx)
    ...
    def backward_G(self):
        ...
        # Identity loss
        if lambda_idt > 0:
            self.idt_A = self.netG_A(self.real_B, self.start_idx)
            self.loss_idt_A = self.criterionIdt(self.idt_A, self.real_B) * lambda_B * lambda_idt
            self.idt_B = self.netG_B(self.real_A, self.start_idx)
        ...
        # self.loss_cycle_A = self.criterionSSIM(self.rec_A_255, self.real_A_255) * lambda_A


        # Backward cycle loss
        self.loss_cycle_B = self.criterionCycle(self.rec_B, self.real_B) * lambda_B
        # self.loss_cycle_B = self.criterionSSIM(self.rec_B_255, self.real_B_255) * lambda_B


        '''
        self.cor_coeLoss
        '''
        # self.loss_cor_coe_GA = Cor_CoeLoss(self.fake_B, self.real_A) * lambda_co_A
        # self.loss_cor_coe_GB = Cor_CoeLoss(self.fake_A, self.real_B) * lambda_co_B

        # Forward SSIM loss
        # self.loss_ssim_A = self.criterionSSIM(self.real_B_255, self.fake_B_255)

        # Backward SSIM loss
        # self.loss_ssim_B = self.criterionSSIM(self.real_A_255, self.fake_A_255)

        # Forward Cycle SSIM loss
        # self.forward_cycle_ssim = self.criterionSSIM(self.rec_A, self.real_A)

        # Backward Cycle SSIM loss
        # self.backward_cycle_ssim = self.criterionSSIM(self.rec_B, self.real_B)

        # combined loss
        self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cycle_A + self.loss_cycle_B + self.loss_idt_A + self.loss_idt_B
        # self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cycle_A + self.loss_cycle_B + self.loss_idt_A + self.loss_idt_B + self.alpha * (self.loss_ssim_A + self.loss_ssim_B)
