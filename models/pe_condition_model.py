...
class PEConditionModel(BaseModel):
    def name(self):
        return 'PEConditionModel'
    ...
    def set_input(self, input):
        ...
        self.condition = input[3].to(self.device)

    def forward(self):
        self.fake_B = self.netG_A(self.real_A, self.start_idx, self.condition)
        self.rec_A = self.netG_B(self.fake_B, self.start_idx, self.condition)

        self.fake_A = self.netG_B(self.real_B, self.start_idx, self.condition)
        self.rec_B = self.netG_A(self.fake_A, self.start_idx, self.condition)
    ...
    def backward_G(self):
        ...
        self.real_A_255 = self.real_A * 127.49 + 127.5
        self.real_B_255 = self.real_B * 127.49 + 127.5
        self.fake_A_255 = self.fake_A * 127.49 + 127.5
        self.fake_B_255 = self.fake_B * 127.49 + 127.5

        # Identity loss
        if lambda_idt > 0:
            # G_A should be identity if real_B is fed.
            self.idt_A = self.netG_A(self.real_B, self.start_idx, self.condition)
            self.loss_idt_A = self.criterionIdt(self.idt_A, self.real_B) * lambda_B * lambda_idt
            # G_B should be identity if real_A is fed.
            self.idt_B = self.netG_B(self.real_A, self.start_idx, self.condition)
        ...
        self.cor_coeLoss
        '''
        self.loss_cor_coe_GA = Cor_CoeLoss(self.fake_B,
                                                    self.real_A) * lambda_co_A  # fake ct & real mr; Evaluate the Generator of ct(G_A)
        self.loss_cor_coe_GB = Cor_CoeLoss(self.fake_A,
