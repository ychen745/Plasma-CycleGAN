...
class Pix2Pix3dConditionModel(BaseModel):
    def name(self):
        return 'Pix2Pix3dConditionModel'
    ...
    def initialize(self, opt):
        ...
        self.netG_A = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf,
                                      opt.netG, opt.norm, not opt.no_dropout, opt.init_type, 0.02, self.gpu_ids, mode=opt.bbbm_mode)
        if self.isTrain:
