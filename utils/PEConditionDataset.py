...
class NiftiDataSet(torch.utils.data.Dataset):
    ...
    def __init__(self, data_path, cross_validation=False, data_split='data_split.csv', nth_fold=0,
        ...
        if cross_validation:
            ...
            with open(data_split) as f:
                for line in f:
                    ...
                    if linelist[1] != nth_fold:
                        self.images_list.append(os.path.join(data_path, 'images', linelist[0]))
                        self.labels_list.append(os.path.join(data_path, 'labels', linelist[0]))
        else:
...
class NiftiDataSet_testing(torch.utils.data.Dataset):
...
class PEConditionDataSet(torch.utils.data.Dataset):

    def __init__(self, data_path, cross_validation=False, data_split='data_split_abeta.csv', nth_fold=0,
                 which_direction='AtoB',
                 transforms=None,
                 shuffle_labels=False,
                 mode='train',
        ...
        print(os.path.join(data_path, data_split))

        df = pd.read_csv(os.path.join(data_path, data_split), index_col=False)
        if cross_validation:
            df = pd.read_csv(os.path.join(data_path, data_split))
            val_group = nth_fold
            test_group = (nth_fold + 1) % 10
            train_images = df[~df['group'].isin([val_group, test_group])]['mri_id']
            train_labels = df[~df['group'].isin([val_group, test_group])]['pet_id']
            val_images = df[df['group'] == val_group]['mri_id']
            val_labels = df[df['group'] == val_group]['pet_id']
            test_images = df[df['group'] == test_group]['mri_id']
            test_labels = df[df['group'] == test_group]['pet_id']

            self.train_images_list = [os.path.join(self.data_path, 'images', ele) for ele in train_images]
            self.train_labels_list = [os.path.join(self.data_path, 'labels', ele) for ele in train_labels]
            self.val_images_list = [os.path.join(self.data_path, 'images', ele) for ele in val_images]
            self.val_labels_list = [os.path.join(self.data_path, 'labels', ele) for ele in val_labels]
            self.test_images_list = [os.path.join(self.data_path, 'images', ele) for ele in test_images]
            self.test_labels_list = [os.path.join(self.data_path, 'labels', ele) for ele in test_labels]
        else:
            df = pd.read_csv(os.path.join(data_path, data_split))
            nth_fold = 0
            val_group = nth_fold
        ...
        self.train_condition_list = list(df[~df['group'].isin([val_group, test_group])][self.condition])
        self.val_condition_list = list(df[df['group'] == val_group][self.condition])
        self.test_condition_list = list(df[df['group'] == test_group][self.condition])

        self.train_images_size = len(self.train_images_list)
        self.train_labels_size = len(self.train_labels_list)
        self.val_images_size = len(self.val_images_list)
        self.val_labels_size = len(self.val_labels_list)
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            ...
            if self.shuffle_labels is True:
                index_B = random.randint(0, self.train_labels_list - 1)
            ...
            if self.shuffle_labels is True:
                index_B = random.randint(0, self.val_labels_list - 1)
            ...
            if self.shuffle_labels is True:
                index_B = random.randint(0, self.test_labels_list - 1)
        ...
        condition = self.train_condition_list[index]

        if 'index' in sample.keys():
            idx = sample['index']
        else:
            print('Error: no index info')
            exit()

        return torch.from_numpy(image_np), torch.from_numpy(label_np), torch.tensor(idx), torch.tensor(condition).unsqueeze(-1)  # this is the final output to feed the network
...
class PEConditionDataSet_testing(torch.utils.data.Dataset):
