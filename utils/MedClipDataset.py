...
class MedClipDataSet(torch.utils.data.Dataset):

    def __init__(self, data_path, cross_validation=False, data_split='data_split_text.csv', nth_fold=0,
        ...
        df = pd.read_csv(data_split, index_col=False)
        train_images = df[(df['group'] != 8) & (df['group'] != 9)]['mri_id']
        train_labels = df[(df['group'] != 8) & (df['group'] != 9)]['pet_id']
        val_images = df[df['group'] == 8]['mri_id']
        val_labels = df[df['group'] == 8]['pet_id']
        test_images = df[df['group'] == 9]['mri_id']
        test_labels = df[df['group'] == 9]['pet_id']
        
        train_texts = list(df[(df['group'] != 8) & (df['group'] != 9)]['text'])
        val_texts = list(df[df['group'] == 8]['text'])
        test_texts = list(df[df['group'] == 9]['text'])

        tokenizer = AutoTokenizer.from_pretrained('Charangan/MedBERT')
        tokenized_train = tokenizer(train_texts, truncation=True, max_length=77, padding=True, return_tensors='pt')
        tokenized_val = tokenizer(val_texts, truncation=True, max_length=77, padding=True, return_tensors='pt')
        tokenized_test = tokenizer(test_texts, truncation=True, max_length=77, padding=True, return_tensors='pt')

        self.train_inputids_list = tokenized_train['input_ids']
        self.val_inputids_list = tokenized_val['input_ids']
        self.test_inputids_list = tokenized_test['input_ids']

        self.train_mask_list = tokenized_train['attention_mask']
        self.val_mask_list = tokenized_val['attention_mask']
        self.test_mask_list = tokenized_test['attention_mask']

        self.train_images_list = [os.path.join(data_path, 'images', ele) for ele in train_images]
        self.train_labels_list = [os.path.join(data_path, 'labels', ele) for ele in train_labels]
        self.val_images_list = [os.path.join(data_path, 'images', ele) for ele in val_images]
        self.val_labels_list = [os.path.join(data_path, 'labels', ele) for ele in val_labels]
        self.test_images_list = [os.path.join(data_path, 'images', ele) for ele in test_images]
        self.test_labels_list = [os.path.join(data_path, 'labels', ele) for ele in test_labels]
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            ...
            if self.shuffle_labels is True:
                index_B = random.randint(0, self.train_labels_size - 1)
                label_path = self.train_labels_list[index_B]
            else:
                label_path = self.train_labels_list[index]
            input_ids = self.train_inputids_list[index]
            attention_mask = self.train_mask_list[index]
        elif self.mode == 'val':
            data_path = self.val_images_list[index]
            if self.shuffle_labels is True:
                index_B = random.randint(0, self.val_labels_size - 1)
                label_path = self.val_labels_list[index_B]
            else:
                label_path = self.val_labels_list[index]
            input_ids = self.val_inputids_list[index]
            attention_mask = self.val_mask_list[index]
        else:
            data_path = self.test_images_list[index]
            if self.shuffle_labels is True:
                index_B = random.randint(0, self.test_labels_size - 1)
                label_path = self.test_labels_list[index_B]
            else:
                label_path = self.test_labels_list[index]
            input_ids = self.test_inputids_list[index]
            attention_mask = self.test_mask_list[index]
        ...
        return torch.from_numpy(image_np), torch.from_numpy(label_np), input_ids, attention_mask

    def __len__(self):
        if self.mode == 'train':
            return len(self.train_images_list)
        elif self.mode == 'val':
            return len(self.val_images_list)
        else:
            return len(self.test_images_list)


class MedClipDataSet_testing(torch.utils.data.Dataset):

    def __init__(self, data_list,label_list,text_list,
                 which_direction='AtoB',
                 transforms=None,
                 mode='test'):
