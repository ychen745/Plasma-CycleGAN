...
class PretrainDataSet(torch.utils.data.Dataset):
    ...
    def __init__(self, data_path, cross_validation=False, data_split='data_split_abeta.csv', nth_fold=0,
        ...
        train_images = images[:len(images) // 5 * 4]
        train_labels = labels[:len(labels) // 5 * 4]
        val_images = images[len(images) // 5 * 4:]
        val_labels = labels[len(labels) // 5 * 4:]
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            ...
            print('Wrong mode in pretraining')
            exit()
...
class PretrainDataSet_testing(torch.utils.data.Dataset):
