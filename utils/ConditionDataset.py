...
class ConditionDataSet(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return torch.from_numpy(image_np), torch.from_numpy(label_np), torch.tensor(condition).unsqueeze(-1)  # this is the final output to feed the network
...
class ConditionDataSet_testing(torch.utils.data.Dataset):
