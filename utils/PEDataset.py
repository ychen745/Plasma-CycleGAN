...
class PEDataSet_testing(torch.utils.data.Dataset):
...
class PEDataSet(torch.utils.data.Dataset):
    ...
    def __init__(self, data_path, cross_validation=False, data_split='data_split.csv', nth_fold=0,
        ...
        if cross_validation:
            df = pd.read_csv(os.path.join(data_path, data_split))
            train_images = df[df['group'] != nth_fold]['mri_id']
            train_labels = df[df['group'] != nth_fold]['pet_id']
            test_images = df[df['group'] == nth_fold]['mri_id']
            test_labels = df[df['group'] == nth_fold]['pet_id']
            ...
            df = pd.read_csv(os.path.join(data_path, data_split))
            train_images = df[df['group'] != 4]['mri_id']
            train_labels = df[df['group'] != 4]['pet_id']
            test_images = df[df['group'] == 4]['mri_id']
            test_labels = df[df['group'] == 4]['pet_id']
    ...
    def __getitem__(self, index):
        ...
        return torch.from_numpy(image_np), torch.from_numpy(label_np), torch.tensor(index)  # this is the final output to feed the network
