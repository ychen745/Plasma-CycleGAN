...
class PetDataSet(torch.utils.data.Dataset):
    ...
    def __init__(self, data_path, cross_validation=False,
        ...
        self.image_list = [os.path.join(data_path, 'FBP', ele) for ele in os.listdir(os.path.join(data_path, 'FBP'))]
        self.label_list = [os.path.join(data_path, 'PIB', ele) for ele in os.listdir(os.path.join(data_path, 'PIB'))]
    ...
    def __getitem__(self, index):
        ...
        if self.shuffle_labels is True:
            index_B = random.randint(0, len(self.label_list) - 1)
            label_path = self.label_list[index_B]
        else:
...
class PetDataSet_testing(torch.utils.data.Dataset):
