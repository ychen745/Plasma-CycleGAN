...
import utils.PEConditionDataset as PEConditionDataset
...
if __name__ == '__main__':
	...
	trainTransforms = [
				...
				PEConditionDataset.RandomCrop((opt.patch_size[0], opt.patch_size[1], opt.patch_size[2]), opt.drop_ratio, min_pixel),
				]
	
	if opt.cross_validation:
		train_set = PEConditionDataSet(opt.data_path, True, opt.data_split, opt.fold, which_direction='AtoB', transforms=trainTransforms, shuffle_labels=False, mode='train', condition=opt.condition)
		val_set = PEConditionDataSet(opt.data_path, True, opt.data_split, opt.fold, which_direction='AtoB', transforms=trainTransforms, shuffle_labels=False, mode='val', condition=opt.condition)
	else:
		train_set = PEConditionDataSet(opt.data_path, False, opt.data_split, 0, which_direction='AtoB', transforms=trainTransforms, shuffle_labels=False, mode='train', condition=opt.condition)
		val_set = PEConditionDataSet(opt.data_path, False, opt.data_split, 0, which_direction='AtoB', transforms=trainTransforms, shuffle_labels=False, mode='val', condition=opt.condition)
	...
	for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):
		...
		# 		fake_pet = model.netG_A(model.real_A, model.start_idx, model.condition)
