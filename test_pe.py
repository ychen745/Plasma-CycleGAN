...
import utils.PEDataset as PEDataset_testing
...
def inference(model, image_path, result_path, resample, resolution, patch_size_x,
    ...
    transforms1 = [
        PEDataset_testing.Resample(resolution, resample)
    ]

    transforms2 = [
        PEDataset_testing.Padding((patch_size_x, patch_size_y, patch_size_z))
    ]
...
if __name__ == '__main__':
    ...
    for f in img_list:
        image_path = os.path.join(opt.data_path, 'images', f)
        result_path = os.path.join(opt.result, f)
        inference(model, image_path, result_path, opt.resample, opt.new_resolution, opt.patch_size[0], opt.patch_size[1], opt.patch_size[2], opt.stride_inplane, opt.stride_layer, 1)
