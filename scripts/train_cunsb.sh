PYTHON_CODE=/scratch/ychen855/mri2pet/CycleGAN_3D/train_cunsb.py
DATA_PATH=/data/hohokam/Yanxi/Data/mri2pet/adni
NAME=cunsb  ### new experiment for unpaired SB_training
MODEL=sb
WEIGHT_SB=1.0
GPU_ID=0
...
### validation control
IF_VAL=True  ### 
VAL_DATA_PATH=./data/hohokam/Yanxi/Data/mri2pet/adni
VAL_BATCH=16
...
python ${PYTHON_CODE}  --DSE_padding_type ${DSC_P} --DSC_n_blocks ${DSC_B} --DSC_number ${DSC_N} --DSC_kernel ${DSC_K} \
 ...
 --if_validation ${IF_VAL} --normD ${NORM_DIS}  --data_path ${DATA_PATH} --name ${NAME} --mode ${MODEL} --lambda_SB ${WEIGHT_SB} \
