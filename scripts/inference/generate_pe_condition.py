...
save_freq = 20
which_epochs = [60, 80, 100, 120, 140, 160, 180, 200]
...
model = 'pe_condition' if training else 'pe_condition_test'
netg = 'pe_condition'
if training:
    python_path = '/scratch/ychen855/mri2pet/CycleGAN_3D/train_pe_condition.py'
else:
    python_path = '/scratch/ychen855/mri2pet/CycleGAN_3D/test_pe_condition.py'
                ...
                for i in range(1):
                    ...
                    with open(job_file, 'w') as f:
                        ...
                        if training:
                            ...
                            if exp_name + '_' + str(which_epoch) + '_' + str(n_epoch) not in os.listdir(gen_root):
                            ...
                            command = 'python ${PYTHON_PATH} --data_path ${DATA_PATH} --data_split ${DATA_SPLIT} --name ${NAME} --condition ${CONDITION} --image ${IMG_DIR} --result ${RES_DIR} --workers ${NWORKERS} --netG ${NETG} --model ${MODEL} --ngf ${NGF} --checkpoints_dir ${CHECKPOINTS_DIR} --which_epoch ${WHICH_EPOCH} --bbbm_mode ${BBBM_MODE}'
                            if cross_validation:
