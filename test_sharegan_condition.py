...
def inference(model, image_path, result_path, resample, resolution, patch_size_x,
    ...
    for i in tqdm(range(len(batches))):
        ...
        model.set_input(batch, condition_tensor)
        model.test()
...
if __name__ == '__main__':
    ...
    #     img_list = df[df['group'] == opt.fold]['mri_id']
    # else:
    #     img_list = df[df['group'] == 4]['mri_id']
    ...
    for i in range(len(img_list)):
        ...
        inference(model, image_path, result_path, opt.resample, opt.new_resolution, opt.patch_size[0], opt.patch_size[1], opt.patch_size[2], opt.stride_inplane, opt.stride_layer, condition_list[i], 1)
